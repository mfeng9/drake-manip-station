...
class IiwaPlanRunner(LeafSystem):
    ...
    #                 duration=3600.0, delta_q=np.zeros(7))
    ...
    def GetCurrentPlan(self, context):
        """
        modified plan dispatcher for running plans interactively
        Either:
            -- add plan to self.kuka_plans_list and let it automatically execute
               (will wait at most 1 second)
            -- directly set self.current_plan and self.current_plan_start_time externally
               (can be useful if desire no wait)
        """
        t = context.get_time()
        ...
        if self.current_plan is None:
            ...
            self.current_plan_start_time = 0.
            return
        ### manually dispatch new plan by checking the end time of previous plan
        ### and current time 
        self.overdue = self.current_plan_start_time + self.current_plan.duration - t  
        if self.overdue + 0.05 < 0:
            ### for arm, if there is a plan supplied by the user, use it
            if len(self.kuka_plans_list) > 0:
                self.current_plan = self.kuka_plans_list.pop(0)
                # print("arm runner: received new plan at t = {}".format(t))
            ### otherwise hold the gripper in place
            else:
                self.current_plan = JointSpacePlanRelative(
                    duration=1.0, delta_q=np.zeros(7))
                # print("arm idle at time = {}".format(context.get_time()))
            ### record new plan start time
            self.current_plan_start_time = context.get_time()

            ### for gripper, if there is a new gripper command, use it
            if len(self.gripper_setpoint_list) > 0:
                self.current_gripper_setpoint = self.gripper_setpoint_list.pop(0)
                print("gripper runner: received new plan at t = {}".format(t))
